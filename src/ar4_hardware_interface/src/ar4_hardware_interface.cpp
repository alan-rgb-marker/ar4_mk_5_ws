#include <ar4_hardware_interface/ar4_hardware_interface.hpp>
#include <sstream>
...
namespace ar4_hardware_interface
{
      ...
      const rclcpp::Duration & /*period*/)
  {
    ...
    for (size_t i = 0; i < info_.joints.size(); i++)
    {
      ...
      jointPositionStm << std::fixed << std::setprecision(2) << radToDeg(joint_positions_[i]);
      ...
      const rclcpp::Duration & /*period*/)
  {
    ...
    {
      std::stringstream jointPositionStm;
      jointPositionStm << std::fixed << std::setprecision(2) << radToDeg(joint_position_commands_[i]);
      logInfo += info_.joints[i].name + ": " + jointPositionStm.str() + " | ";
    }
    RCLCPP_INFO_THROTTLE(logger_, clock_, 500, logInfo.c_str());
  ...
  hardware_interface::CallbackReturn ARHardwareInterface::on_shutdown(const rclcpp_lifecycle::State & /*previous_state*/)
  {
...
PLUGINLIB_EXPORT_CLASS(ar4_hardware_interface::ARHardwareInterface, hardware_interface::SystemInterface)
