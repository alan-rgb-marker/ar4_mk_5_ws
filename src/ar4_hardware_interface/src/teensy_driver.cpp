...
namespace ar4_hardware_interface
{
  ...
  void TeensyDriver::init(std::string port, int baudrate, int num_joints)
  {
    ...
    {
      serial_port_.close();                                        // Close immediately to reset connection, will reopen in exchange()
      std::this_thread::sleep_for(std::chrono::milliseconds(500)); // Wait for connection to reset
      serial_port_.open(port, ec);
  ...
  void TeensyDriver::close()
  {
    if (serial_port_.is_open())
    {
      serial_port_.close();
      RCLCPP_INFO(logger_, "Closed serial port connection");
    }
  }
