...
class DepthSubscriber(Node):
    ...
    def tf_callback(self):
        ...
        if self.k_received:
            self.x = (transform.transform.translation.x - self.k[2]) * self.distance / self.k[0]  # 假設 k[0] 是焦距 fx k[2] 是主點 cx
            self.y = (transform.transform.translation.y - self.k[5]) * self.distance / self.k[4]  # 假設 k[4] 是焦距 fy k[5] 是主點 cy
            z = transform.transform.translation.z - self.distance
            if z < 0.1562 and self.gripper_state <= 0.1:
                self.z = 0.1562
            elif self.z < 0.1515 and self.gripper_state > 0.1:
                self.z = 0.1562
            else:
                self.z = z
